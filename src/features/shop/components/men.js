...
const Men = ({ disabled, price, sort }) => {
    // place the items that should be displayed in list
    let list = sortList(
        Object.values(items).filter((item) => item.gender !== "women"), 
        0.5315, 
        11.1523, 
        sort
    );
    ...
    return <>
        <div className="split-row mb-40">
            <div>
                <h2>Men's Watches</h2>
                <br />
                <TitleRow amount={amount} total={total} name="Men" />
...
const mapStateToProps = (state) => {
    return {
        disabled: [...Object.entries(state.filterReducer.data).map(([id,v]) => {
            if(id.includes("women"))return null;
